    ...
    <div className="p-4 flex items-start gap-3 hover:bg-surface-secondary/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-muted-foreground dark:text-slate-300">
          ...
          {phraseParts.map((seg: { text: string; isAnchor: boolean }, i: number) => {
            if (!seg.isAnchor) {
              return (
                <span key={`p-${i}`}>
                  {seg.text}
...
export default function ProjectActivityTab({ projectId }: { projectId: string }) {
  ...
  const q = useQuery<ActivityResponse, Error, ActivityResponse>({
    queryKey: ['activity', { projectId, page, pageSize: PAGE_SIZE }],
    queryFn: () => fetchActivity({ projectId, page, pageSize: PAGE_SIZE }),
    staleTime: 30_000,
    retry: 1,
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 border-b border-border flex items-start justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">Actividad del proyecto</h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Todos los eventos dentro de este proyecto.
                {source ? <span className="ml-2">({source === 'auditoria' ? 'Auditoría SQL' : 'Vista combinada'})</span> : null}
              </p>
            </div>
          </div>
          <div className="text-xs text-muted-foreground pt-0.5">
            {total === 0
              ? 'Sin eventos'
              : `Mostrando ${from}-${to} de ${total} ${total === 1 ? 'evento' : 'eventos'}`}
          </div>
        </div>
      ...
      <div className="p-3 border-t border-border flex items-center justify-between gap-3 text-xs text-muted-foreground shrink-0">
        <div>
